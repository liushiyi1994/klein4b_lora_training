...
def build_marble_pair_prompt(pair_caption: str) -> str:
    ...
    return (
        ...
        "blank white sculpted "
        "stone eyes or closed carved eyelids; no pupils, no irises, no colored "
        "eyes, no catchlights. "
        "Hair must be carved from the same marble as the face. Use hairstyle "
        "only as sculptural shape with broad chiseled grooves, solid stone "
        "masses, and marble veins continuing through the hair; ignore natural "
        "hair color from the reference and preserve only the hairstyle "
        "silhouette, braid shapes, and carved lock structure; no individual "
        "hair strands, no natural hair color, no glossy hair, no soft hair "
        "strands."
    )
